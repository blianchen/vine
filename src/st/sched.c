...
void _st_vp_schedule(void) {
	...
	printf("_st_vp_schedule ---- _st_active_count=%d, flags=%d, thread=%llu \n", _st_active_count, thread->flags, thread);

	/* Resume the thread */
	thread->state = _ST_ST_RUNNING;
//	_ST_RESTORE_CONTEXT(thread);
//    printf("------------------_ST_RESTORE_CONTEXT %d \n", thread->flags);
...
void st_thread_exit(void *retval) {
	...
	printf("st_thread_exit ----_st_active_count=%d, flags=%d, thread=%llu \n", _st_active_count, thread->flags, thread);
...
void _st_thread_main(void) {
	...
	printf("_st_thread_main ---- flags=%d, thread=%llu \n", thread->flags, thread);
...
//	printf("create thread setjmp start ^^^^^^^^^^^^^^^^ %d \n", thread->flags);
	if (MD_SETJMP((thread)->context)) {
//		printf("create thread setjmp in ^^^^^^^^^^^***** \n");
		_st_thread_main();
	}
	...
	printf("st_thread_create ---- flags=%d, thread=%llu \n", thread->flags, thread);

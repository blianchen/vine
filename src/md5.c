...
unsigned int inputLen; /* length of input block */
{
	 ...
	 */
	if (inputLen >= partLen) {
		R_memcpy((unsigned char*)&context->buffer[index], (unsigned char*)input, partLen);
	...
	R_memcpy((unsigned char*)&context->buffer[index], (unsigned char*)&input[i], inputLen - i);
}
...
MD5_CTX *context;               /* context */
{
	...
	R_memset((unsigned char*)context, 0, sizeof(*context));
}
...
unsigned char block[64];
{
	...
	R_memset((unsigned char*)x, 0, sizeof(x));
}
...
unsigned char* Block1;                 /* first block */
unsigned char* Block2;                 /* second block */
unsigned int len;               /* length of blocks */
{
	if(len != 0) {
